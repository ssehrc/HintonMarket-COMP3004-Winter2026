...
void WaitlistsWindow::refreshWaitlists() {
    vendor->waitlists.clear();
    // Always reload from the database
    QSqlQuery query;
    query.prepare("SELECT weekId, category FROM Waitlist WHERE vendorId = ?");
    query.addBindValue(vendor->id);
    if(query.exec()){
        while(query.next()){
            QString weekId = query.value(0).toString();
            QString category = query.value(1).toString();
            vendor->waitlists.push_back(category + "-" + weekId);
        }
    }else{
        qWarning() << "Failed to load waitlists: " << query.lastError().text();
    }
    table->clearContents();
...
void WaitlistsWindow::showEvent(QShowEvent* event) {
    refreshWaitlists();
